; NOTE: Assertions have been autogenerated by utils/update_test_checks.py
; RUN: opt -S -linearize-cfg %s | FileCheck %s

; Attempt to see multiple backedges from the same loop block.

define void @backedge2() {
; CHECK-LABEL: @backedge2(
; CHECK-NEXT:  dummy.idom:
; CHECK-NEXT:    br label [[ENTRY_GUARD:%.*]]
; CHECK:       entry.guard:
; CHECK-NEXT:    br label [[ENTRY:%.*]]
; CHECK:       entry:
; CHECK-NEXT:    store volatile i32 1, i32 addrspace(1)* undef
; CHECK-NEXT:    [[COND0:%.*]] = load volatile i1, i1 addrspace(1)* undef
; CHECK-NEXT:    [[TMP0:%.*]] = select i1 [[COND0]], i32 1, i32 2
; CHECK-NEXT:    br label [[HEADER0_GUARD:%.*]]
; CHECK:       header0.guard:
; CHECK-NEXT:    [[TMP1:%.*]] = phi i32 [ [[TMP5:%.*]], [[LOOP_HEADER0_GUARD_CRIT_EDGE:%.*]] ], [ undef, [[ENTRY]] ]
; CHECK-NEXT:    [[HEADER1_LOAD10:%.*]] = phi i32 [ [[HEADER1_LOAD11:%.*]], [[LOOP_HEADER0_GUARD_CRIT_EDGE]] ], [ undef, [[ENTRY]] ]
; CHECK-NEXT:    [[GUARD_VAR37:%.*]] = phi i32 [ [[GUARD_VAR38:%.*]], [[LOOP_HEADER0_GUARD_CRIT_EDGE]] ], [ [[TMP0]], [[ENTRY]] ]
; CHECK-NEXT:    [[PHI_PH1:%.*]] = phi i32 [ [[PHI_PH:%.*]], [[LOOP_HEADER0_GUARD_CRIT_EDGE]] ], [ undef, [[ENTRY]] ]
; CHECK-NEXT:    [[GUARD_VAR:%.*]] = phi i32 [ [[TMP5]], [[LOOP_HEADER0_GUARD_CRIT_EDGE]] ], [ [[TMP0]], [[ENTRY]] ]
; CHECK-NEXT:    [[PREV_GUARD:%.*]] = icmp eq i32 [[GUARD_VAR]], 1
; CHECK-NEXT:    br i1 [[PREV_GUARD]], label [[HEADER0:%.*]], label [[LOOP_GUARD:%.*]]
; CHECK:       header0:
; CHECK-NEXT:    [[HEADER0_LOAD:%.*]] = load volatile i32, i32 addrspace(1)* undef
; CHECK-NEXT:    br label [[LOOP_GUARD]]
; CHECK:       header1.guard:
; CHECK-NEXT:    [[TMP2:%.*]] = phi i32 [ [[TMP4:%.*]], [[LOOP:%.*]] ], [ [[TMP5]], [[LOOP_HEADER0_GUARD_CRIT_EDGE]] ]
; CHECK-NEXT:    [[GUARD_VAR9:%.*]] = phi i32 [ [[TMP4]], [[LOOP]] ], [ [[GUARD_VAR4:%.*]], [[LOOP_HEADER0_GUARD_CRIT_EDGE]] ]
; CHECK-NEXT:    [[GUARD_VAR3:%.*]] = phi i32 [ [[TMP4]], [[LOOP]] ], [ [[GUARD_VAR38]], [[LOOP_HEADER0_GUARD_CRIT_EDGE]] ]
; CHECK-NEXT:    [[LAST:%.*]] = icmp eq i32 [[GUARD_VAR9]], 2
; CHECK-NEXT:    br i1 [[LAST]], label [[HEADER1:%.*]], label [[HEADER1_SPLIT:%.*]]
; CHECK:       header1:
; CHECK-NEXT:    [[HEADER1_LOAD:%.*]] = load volatile i32, i32 addrspace(1)* undef
; CHECK-NEXT:    br label [[HEADER1_SPLIT]]
; CHECK:       header1.split:
; CHECK-NEXT:    [[HEADER1_LOAD12:%.*]] = phi i32 [ [[HEADER1_LOAD11]], [[HEADER1_GUARD:%.*]] ], [ [[HEADER1_LOAD]], [[HEADER1]] ]
; CHECK-NEXT:    br label [[LOOP_GUARD]]
; CHECK:       loop.guard:
; CHECK-NEXT:    [[TMP3:%.*]] = phi i32 [ [[TMP2]], [[HEADER1_SPLIT]] ], [ [[TMP1]], [[HEADER0]] ], [ [[TMP1]], [[HEADER0_GUARD]] ]
; CHECK-NEXT:    [[HEADER1_LOAD11]] = phi i32 [ [[HEADER1_LOAD12]], [[HEADER1_SPLIT]] ], [ [[HEADER1_LOAD10]], [[HEADER0]] ], [ [[HEADER1_LOAD10]], [[HEADER0_GUARD]] ]
; CHECK-NEXT:    [[GUARD_VAR36:%.*]] = phi i32 [ [[GUARD_VAR3]], [[HEADER1_SPLIT]] ], [ [[GUARD_VAR37]], [[HEADER0]] ], [ [[GUARD_VAR37]], [[HEADER0_GUARD]] ]
; CHECK-NEXT:    [[GUARD_VAR2:%.*]] = phi i32 [ [[GUARD_VAR3]], [[HEADER1_SPLIT]] ], [ 3, [[HEADER0]] ], [ [[GUARD_VAR]], [[HEADER0_GUARD]] ]
; CHECK-NEXT:    [[PHI_PH]] = phi i32 [ [[HEADER1_LOAD12]], [[HEADER1_SPLIT]] ], [ [[HEADER0_LOAD]], [[HEADER0]] ], [ [[PHI_PH1]], [[HEADER0_GUARD]] ]
; CHECK-NEXT:    [[BE_GUARD:%.*]] = icmp eq i32 [[GUARD_VAR2]], 3
; CHECK-NEXT:    br i1 [[BE_GUARD]], label [[LOOP]], label [[LOOP_HEADER0_GUARD_CRIT_EDGE]]
; CHECK:       loop:
; CHECK-NEXT:    [[PHI:%.*]] = phi i32 [ [[PHI_PH]], [[LOOP_GUARD]] ]
; CHECK-NEXT:    store volatile i32 [[PHI]], i32 addrspace(1)* undef
; CHECK-NEXT:    [[COND1:%.*]] = load volatile i1, i1 addrspace(1)* undef
; CHECK-NEXT:    [[TMP4]] = select i1 [[COND1]], i32 3, i32 2
; CHECK-NEXT:    br i1 [[COND1]], label [[LOOP_HEADER0_GUARD_CRIT_EDGE]], label [[HEADER1_GUARD]]
; CHECK:       loop.header0.guard_crit_edge:
; CHECK-NEXT:    [[TMP5]] = phi i32 [ [[TMP3]], [[LOOP_GUARD]] ], [ [[TMP4]], [[LOOP]] ]
; CHECK-NEXT:    [[GUARD_VAR38]] = phi i32 [ [[GUARD_VAR36]], [[LOOP_GUARD]] ], [ [[TMP4]], [[LOOP]] ]
; CHECK-NEXT:    [[GUARD_VAR4]] = phi i32 [ [[GUARD_VAR2]], [[LOOP_GUARD]] ], [ [[TMP4]], [[LOOP]] ]
; CHECK-NEXT:    [[PREV_GUARD5:%.*]] = icmp eq i32 [[GUARD_VAR4]], 1
; CHECK-NEXT:    br i1 [[PREV_GUARD5]], label [[HEADER0_GUARD]], label [[HEADER1_GUARD]]
;
entry:
  store volatile i32 1, i32 addrspace(1)* undef
  %cond0 = load volatile i1, i1 addrspace(1)* undef
  br i1 %cond0, label %header0, label %header1

header0:
  %header0.load = load volatile i32, i32 addrspace(1)* undef
  br label %loop

header1:
  %header1.load = load volatile i32, i32 addrspace(1)* undef
  br label %loop

loop:
  %phi = phi i32 [ %header0.load, %header0 ], [ %header1.load, %header1 ]
  store volatile i32 %phi, i32 addrspace(1)* undef
  %cond1 = load volatile i1, i1 addrspace(1)* undef
  br i1 %cond1, label %header0, label %header1
}

define void @self_loop_2x() {
; CHECK-LABEL: @self_loop_2x(
; CHECK-NEXT:  entry:
; CHECK-NEXT:    store volatile i32 1, i32 addrspace(1)* undef
; CHECK-NEXT:    [[COND0:%.*]] = load volatile i1, i1 addrspace(1)* undef
; CHECK-NEXT:    br label [[LOOP:%.*]]
; CHECK:       loop:
; CHECK-NEXT:    [[PHI:%.*]] = phi i32 [ 0, [[ENTRY:%.*]] ], [ 1, [[LOOP]] ], [ 1, [[LOOP]] ]
; CHECK-NEXT:    store volatile i32 [[PHI]], i32 addrspace(1)* undef
; CHECK-NEXT:    [[COND1:%.*]] = load volatile i1, i1 addrspace(1)* undef
; CHECK-NEXT:    br i1 [[COND1]], label [[LOOP]], label [[LOOP]]
;
entry:
  store volatile i32 1, i32 addrspace(1)* undef
  %cond0 = load volatile i1, i1 addrspace(1)* undef
  br label %loop

loop:
  %phi = phi i32 [ 0, %entry ], [ 1, %loop ], [ 1, %loop ]
  store volatile i32 %phi, i32 addrspace(1)* undef
  %cond1 = load volatile i1, i1 addrspace(1)* undef
  br i1 %cond1, label %loop, label %loop
}
