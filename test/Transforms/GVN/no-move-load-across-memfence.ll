...
; CHECK-LABEL: @different_address_space(
; CHECK: phi <2 x float> [ %tmp25.pre, %if.then ], [ %tmp4, %entry ]
; CHECK: tail call void @__amdil_barrier_global()
...
entry:
  ...
  %tmp4 = load <2 x float> addrspace(1)* %arrayidx3, align 8
  store <2 x float> %tmp4, <2 x float> addrspace(3)* %arrayidx, align 8
  tail call void @__amdil_barrier_global() #3
  %cmp = icmp ult i32 %1, 5
  br i1 %cmp, label %if.then, label %if.end
