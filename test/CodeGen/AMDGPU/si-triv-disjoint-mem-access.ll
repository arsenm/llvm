; RUN: llc -march=amdgcn -mcpu=bonaire -verify-machineinstrs -enable-misched < %s | FileCheck -check-prefix=FUNC -check-prefix=CI %s
; RUN: llc -march=amdgcn -mcpu=bonaire -mattr=+load-store-opt -verify-machineinstrs -enable-misched < %s | FileCheck -check-prefix=FUNC -check-prefix=CI-OPT %s
...
; CI-OPT: ds_read_b32 {{v[0-9]+}}, {{v[0-9]+}}{{$}}
; CI-OPT: ds_read2_b32 {{v\[[0-9]+:[0-9]+\]}}, {{v[0-9]+}} offset0:1 offset1:2
...
; XXX: Should be able to reorder this, but the loads count as ordered
...
; FUNC-LABEL: {{^}}reorder_local_offsets_v:
; CI-OPT: ds_read2_b32 {{v\[[0-9]+:[0-9]+\]}}, {{v[0-9]+}} offset0:100 offset1:101
; CI-OPT: ds_write_b32 {{v[0-9]+}}, {{v[0-9]+}} offset:400
; CI-OPT: ds_write2_b32 {{v[0-9]+}}, {{v[0-9]+}}, {{v[0-9]+}} offset0:3 offset1:101
; CI-OPT: buffer_store_dword
; CI-OPT: s_endpgm
...
define void @reorder_local_offsets_v(i32 addrspace(1)* nocapture %out, i32 addrspace(1)* noalias nocapture readnone %gptr, i32 addrspace(3)* noalias nocapture %ptr) #0 {
  %tid = call i32 @llvm.r600.read.tidig.x() #3
  %ptr0 =  getelementptr inbounds i32, i32 addrspace(3)* %ptr, i32 %tid
...
;   %ptr0 = load i32, i32 addrspace(3)*, i32 addrspace(3)* addrspace(3)* @stored_lds_ptr, align 4

;   %ptr1 = getelementptr inbounds i32, i32, i32 addrspace(3)* %ptr0, i32 1
;   %ptr2 = getelementptr inbounds i32, i32, i32 addrspace(3)* %ptr0, i32 2
