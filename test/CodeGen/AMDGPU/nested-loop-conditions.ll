...
; IR: bb5:
; IR-NEXT: %0 = phi i64 [ %loop.phi, %bb10 ], [ 0, %bb ]
; IR-NEXT: %tmp6 = phi i32 [ 0, %bb ], [ %tmp11, %bb10 ]
; IR-NEXT: %tmp7 = icmp eq i32 %tmp6, 1
; IR-NEXT: %1 = call { i1, i64 } @llvm.amdgcn.if(i1 %tmp7)
; IR-NEXT: %2 = extractvalue { i1, i64 } %1, 0
; IR-NEXT: %3 = extractvalue { i1, i64 } %1, 1
; IR-NEXT: br i1 %2, label %bb8, label %Flow

; IR: bb8:
; IR-NEXT: %4 = call i64 @llvm.amdgcn.break(i64 %0)
; IR-NEXT: br label %bb13

; IR: bb10:
; IR-NEXT: %loop.phi = phi i64 [ %7, %Flow ]
; IR-NEXT: %tmp11 = phi i32 [ %6, %Flow ]
; IR-NEXT: %5 = call i1 @llvm.amdgcn.loop(i64 %loop.phi)
; IR-NEXT: br i1 %5, label %bb23, label %bb5

; IR: Flow:
; IR-NEXT: %loop.phi1 = phi i64 [ %loop.phi2, %bb4 ], [ %0, %bb5 ]
; IR-NEXT: %6 = phi i32 [ %tmp21, %bb4 ], [ undef, %bb5 ]
; IR-NEXT: %7 = call i64 @llvm.amdgcn.else.break(i64 %3, i64 %loop.phi1)
; IR-NEXT: call void @llvm.amdgcn.end.cf(i64 %3)
; IR-NEXT: br label %bb10

; IR: bb13:
; IR-NEXT: %loop.phi3 = phi i64 [ %loop.phi4, %bb3 ], [ %4, %bb8 ]
...
; IR: bb20:
; IR-NEXT: %loop.phi4 = phi i64 [ %0, %bb16 ], [ %0, %bb13 ]
; IR-NEXT: %loop.phi2 = phi i64 [ %0, %bb16 ], [ %loop.phi3, %bb13 ]
...
; IR: Flow7:
; IR-NEXT: call void @llvm.amdgcn.end.cf(i64 %18)
; IR-NEXT: %0 = call { i1, i64 } @llvm.amdgcn.if(i1 %16)
; IR-NEXT: %1 = extractvalue { i1, i64 } %0, 0
; IR-NEXT: %2 = extractvalue { i1, i64 } %0, 1
; IR-NEXT: br i1 %1, label %bb4.bb13_crit_edge, label %Flow8

; IR: Flow1:
; IR-NEXT: %loop.phi = phi i64 [ %loop.phi9, %Flow6 ], [ %8, %bb14 ]
; IR-NEXT: %14 = phi <4 x i32> [ %30, %Flow6 ], [ undef, %bb14 ]
; IR-NEXT: %15 = phi i32 [ %31, %Flow6 ], [ undef, %bb14 ]
; IR-NEXT: %16 = phi i1 [ %32, %Flow6 ], [ false, %bb14 ]
; IR-NEXT: %17 = phi i1 [ false, %Flow6 ], [ %9, %bb14 ]
; IR-NEXT: %18 = call i64 @llvm.amdgcn.else.break(i64 %12, i64 %loop.phi)
; IR-NEXT: call void @llvm.amdgcn.end.cf(i64 %12)
; IR-NEXT: %19 = call i1 @llvm.amdgcn.loop(i64 %18)
; IR-NEXT: br i1 %19, label %Flow7, label %bb14

; IR: Flow2:
; IR-NEXT: %loop.phi10 = phi i64 [ %loop.phi11, %Flow5 ], [ %13, %bb16 ]
; IR-NEXT: %20 = phi <4 x i32> [ %30, %Flow5 ], [ undef, %bb16 ]
; IR-NEXT: %21 = phi i32 [ %31, %Flow5 ], [ undef, %bb16 ]
; IR-NEXT: %22 = phi i1 [ %32, %Flow5 ], [ false, %bb16 ]
; IR-NEXT: %23 = phi i1 [ false, %Flow5 ], [ false, %bb16 ]
; IR-NEXT: %24 = phi i1 [ false, %Flow5 ], [ %9, %bb16 ]
; IR-NEXT: %25 = call { i1, i64 } @llvm.amdgcn.if(i1 %24)
; IR-NEXT: %26 = extractvalue { i1, i64 } %25, 0
; IR-NEXT: %27 = extractvalue { i1, i64 } %25, 1
; IR-NEXT: br i1 %26, label %bb21, label %Flow3

; IR: bb21:
; IR: %tmp12 = icmp slt i32 %tmp11, 9
; IR-NEXT: %28 = xor i1 %tmp12, true
; IR-NEXT: %29 = call i64 @llvm.amdgcn.if.break(i1 %28, i64 %8)
; IR-NEXT: br label %Flow3

; IR: Flow3:
; IR-NEXT: %loop.phi11 = phi i64 [ %8, %bb21 ], [ %8, %Flow2 ]
; IR-NEXT: %loop.phi9 = phi i64 [ %29, %bb21 ], [ %loop.phi10, %Flow2 ]
; IR-NEXT: %30 = phi <4 x i32> [ %tmp9, %bb21 ], [ %20, %Flow2 ]
; IR-NEXT: %31 = phi i32 [ %tmp10, %bb21 ], [ %21, %Flow2 ]
; IR-NEXT: %32 = phi i1 [ %28, %bb21 ], [ %22, %Flow2 ]
; IR-NEXT: call void @llvm.amdgcn.end.cf(i64 %27)
; IR-NEXT: br i1 %23, label %bb31.loopexit, label %Flow4
