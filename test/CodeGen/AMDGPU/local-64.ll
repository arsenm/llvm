...
; RUN: llc -march=amdgcn -mcpu=bonaire -verify-machineinstrs < %s | FileCheck -check-prefix=CI -check-prefix=GCN -check-prefix=CIVI %s
; RUN: llc -march=amdgcn -mcpu=tonga -mattr=-flat-for-global -verify-machineinstrs< %s | FileCheck -check-prefix=GCN -check-prefix=CIVI %s
...
; CIVI: s_add_i32 [[ADDR:s[0-9]+]], s{{[0-9]+}}, 0x10000
; GCN: v_mov_b32_e32 [[VREGADDR:v[0-9]+]], [[ADDR]]
...
; CIVI: ds_write_b128 v{{[0-9]+}}, {{v\[[0-9]+:[0-9]+\]}} offset:112
; GCN: s_endpgm
...
; CIVI: ds_write_b128 v{{[0-9]+}}, {{v\[[0-9]+:[0-9]+\]$}}
; GCN: s_endpgm
