; XFAIL: *
; RUN: llc -O0 -march=r600 -mcpu=SI < %s | FileCheck -check-prefix=SI -check-prefix=COMMON %s
; RUN: llc -O0 -march=r600 -mcpu=redwood < %s | FileCheck -check-prefix=EG -check-prefix=COMMON %s

; COMMON-LABEL: @extrastride:
define void @extrastride(i8 addrspace(1)* nocapture %main, i32 %main_stride) #0 {
entry:
