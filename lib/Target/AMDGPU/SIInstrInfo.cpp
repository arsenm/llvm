...
void SIInstrInfo::setBranchDestBlock(MachineInstr &MI,
                                     MachineBasicBlock &NewDestBB) const {
  return MI.getOperand(0).setMBB(&NewDestBB);
}

static unsigned getInverseBrOpcode(unsigned Opc) {
  switch (Opc) {
  ...
  default:
    llvm_unreachable("invalid conditional branch opcode");
  }
}

unsigned SIInstrInfo::insertInvertedConditionalBranch(
  MachineBasicBlock &SrcBB, MachineBasicBlock::iterator InsPt,
  const DebugLoc &DL, const MachineInstr &OldBr,
  MachineBasicBlock &DestBB) const {
  BuildMI(SrcBB, InsPt, DL, get(getInverseBrOpcode(OldBr.getOpcode())))
    .addMBB(&DestBB);
  return 4;
}

unsigned SIInstrInfo::insertUnconditionalBranch(MachineBasicBlock &MBB,
                                                MachineBasicBlock &DestBB,
                                                const DebugLoc &DL,
                                                int64_t BrOffset,
                                                RegScavenger *RS) const {
  if (isIntN(BranchOffsetBits, (BrOffset / 4) - 1)) {
