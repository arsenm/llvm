...
void llvm::convertMemCpyToLoop(MemCpyInst *Memcpy) {
...
void llvm::convertMemMoveToLoop(MemMoveInst *Memmove) {
...
void llvm::convertMemSetToLoop(Instruction *ConvertedInst,
  ...
  Function *F = OrigBB->getParent();
  BasicBlock *NewBB =
      OrigBB->splitBasicBlock(ConvertedInst, "split");
  BasicBlock *LoopBB
...
void llvm::convertMemSetToLoop(MemSetInst *Memset) {
