...
namespace {
  ...
  private:
    ...
    void assignDanglingDebugValues(MachineInstr &Def, unsigned VirtReg,
                                   MCPhysReg Reg);
...
void RegAllocFast::assignDanglingDebugValues(MachineInstr &Definition,
                                             unsigned VirtReg, MCPhysReg Reg) {
...
void RegAllocFast::allocVirtReg(MachineInstr &MI, LiveReg &LR,
                                unsigned Hint1, bool LookAtPhysRegUses) {
  ...
  unsigned Hint0 = traceCopies(VirtReg);
...
void RegAllocFast::defineVirtReg(MachineInstr &MI, unsigned OpNum,
                                 unsigned VirtReg, bool LookAtPhysRegUses) {
    ...
    if (HasVRegDef) {
      ...
      if (NeedToAssignLiveThroughs) {
        ...
        llvm::sort(DefOperandIndexes.begin(), DefOperandIndexes.end(),
          ...
          // Identify regclass that are easy to use up completely just in
          // this instruction.
